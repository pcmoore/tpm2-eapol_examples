...
function usage() {
	echo "usage: atomix-tpm-read-passwd.sh <pubkey.pem> <ea.policy.digest>"
	exit 1
}
...
luks_len=$(nvindex_len $TPM_NV_LUKS)
...
notice "LUKS password [$luks_len bytes]"
(tpm2_nvread -P "session:$tdir/tpm_session_a" -s $luks_len $TPM_NV_LUKS;
 echo "")
