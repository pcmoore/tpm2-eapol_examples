...
function usage() {
	echo "usage: atomix-tpm-setup.sh <pubkey.pem> <pcr_prod.bin> <pcr_tpm.bin>"
...
tdir=$(mktemp -d -t atomix-tpm-XXXXXXXX)
...
# NOTE: this policy file must be signed afterwards
pol_luks="./tpm_luks.policy"

notice "setting TPM_NV_LUKS"

# create the luks passwd
luks="insieme-$(rng_bytes 16)"
luks_len=$(str_len $luks)
...
tpm2_policyauthorize -S $tdir/tpm_session_b -n $tdir/pubkey_name_a \
	-L $tdir/tpm_policy_b
tpm2_nvdefine -a "ownerwrite|ownerread|policyread" \
	-P $passwd -L $tdir/tpm_policy_b -s $luks_len $TPM_NV_LUKS
tpm2_flushcontext $tdir/tpm_session_b

# set the nvindex value
echo -n $luks | tpm2_nvwrite -C o -P $passwd -i- $TPM_NV_LUKS
...
notice "nvindices TPM($TPM_NV_PASSWD) VER($TPM_NV_VERSION) LUKS($TPM_NV_LUKS)"
notice "policy TPM($pol_passwd) LUKS($pol_luks)"
exit 0
